...
SC2DATA      ?= data/StarCraft2
export SC2_DATA := $(SC2DATA)
...
run-sc2: $(SC2_BINARY)
	$(SC2_BINARY) -data "$$SC2_DATA" +map TRaynor01 $(ARGS)

build-run-sc2: opensc2
	$(SC2_BINARY) -data "$$SC2_DATA" +map TRaynor01
...
$(eval $(call unity_lib_schema,$(RENDERER_SC2_LIB),$(RENDERER_BASE_DEPS) $(call CSRC,renderer $(SC2_DIR)/renderer) $(SC2_COMMON_SRCS),renderer-sc2,renderer $(SC2_DIR)/renderer,,$(SC2_CFLAGS),common/mpq.c $(SC2_DIR)/common/sc2_dds.c $(SC2_DIR)/common/sc2_m3.c,$(RENDERER_SHARED_LIBS) $(SC2_XML_LIBS)))
...
$(eval $(call test_schema,test-mdx-model-lifecycle,,$(CFLAGS) -Itests,$(BIN_DIR)/test_mdx_model_lifecycle$(EXE_EXT),tests/test_mdx_model_lifecycle.c,,,))
...
test: test-client-model-lifecycle test-mdx-model-lifecycle test-sc2-renderer-model-dispatch test-sc2-tabletop-models
...
$(SC2_HUD_LIVE_BIN): $(SC2_HUD_LIVE_SRC) | $(BIN_DIR)
	$(CC) $(CFLAGS) -I. -Itests -DSC2_BINARY=\"$(SC2_BINARY)\" \
...
test-sc2-live: opensc2 $(SC2_HUD_LIVE_BIN)
	@if [ ! -d "$$SC2_DATA" ]; then \
	    echo "SKIP test-sc2-live: $$SC2_DATA not found"; exit 0; \
	else \
	    $(SC2_HUD_LIVE_BIN); \
	fi

.PHONY: default build shared tools font $(TOOL_NAMES) diag clean download renderer-wow game-wow ui-wow openwow renderer-sc2 game-sc2 opensc2 run run-sc2 build-run-sc2 m2tool-wow-orcmale-player install-wow test-client-model-lifecycle test-mdx-model-lifecycle test-sc2-renderer-model-dispatch test-wow-appearance test-wow-combat test-wow-abilities test-wow-game test-wow-ui test-wow-assets test-sc2 test-sc2-assets test-sc2-live test-sc2-tabletop-assets test-sc2-tabletop-models test-sc2-tabletop-runtime $(WC3_PHONY)
