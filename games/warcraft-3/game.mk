...
$(eval $(call unity_lib_schema,$(RENDERER_LIB),$(RENDERER_BASE_DEPS) $(call CSRC,renderer $(WC3_DIR)/renderer) $(SC2_DIR)/common/sc2_dds.c,renderer,renderer $(WC3_DIR)/renderer,,$(WC3_CFLAGS),common/mpq.c $(SC2_DIR)/common/sc2_dds.c,$(RENDERER_SHARED_LIBS)))
...
TEST_SRCS := \
	...
	! -name 'test_bz_tabletop_audio.c' \
	! -name 'test_bz_tabletop_audio_main.c' \
	! -name 'test_bz_tabletop_catalog.c' \
	! -name 'test_bz_tabletop_catalog_main.c' \
...
test: test-assets $(SHARED_LIB) $(JASS_LIB) $(SHEET_LIB) | $(BIN_DIR)
	...
	@$(MAKE) test-bz-tabletop-audio
	@$(MAKE) test-bz-tabletop-catalog
	@$(MAKE) test-sc2
	@$(MAKE) test-sc2-tabletop-assets
...
$(eval $(call test_schema,test-bz-tabletop-lifecycle,test-assets $(SHARED_LIB) $(SHEET_LIB),$(TEST_CFLAGS) -Iplatform/tabletop/bridge,$(BIN_DIR)/test_bz_tabletop_lifecycle$(EXE_EXT),$(WC3_TEST_DIR)/test_bz_tabletop_lifecycle_main.c $(WC3_TEST_DIR)/test_bz_tabletop_lifecycle.c platform/tabletop/bridge/bz_tabletop_lifecycle.c common/bz_runtime.c common/common.c common/cmd.c common/cvar.c common/msg.c common/net.c common/mpq.c,-lsheet -lshared -lm -lz -lpthread,))
# Layer 2 tabletop transport ABI: links the REAL client/cl_parse.c and the REAL
# shared, platform-neutral tabletop headless screen/UI-wire glue
# (cl_scrn_tabletop_null.c) - not a second, parallel test-only reimplementation
# of either - alongside the REAL
...
$(eval $(call test_schema,test-bz-tabletop-transport,,$(TEST_CFLAGS) -Iplatform/bridge -Iplatform/tabletop/client,$(BIN_DIR)/test_bz_tabletop_transport$(EXE_EXT),$(WC3_TEST_DIR)/test_bz_tabletop_transport_main.c $(WC3_TEST_DIR)/test_bz_tabletop_transport.c $(WC3_TEST_DIR)/test_bz_tabletop_transport_client.c $(WC3_TEST_DIR)/test_bz_tabletop_transport_stubs.c platform/bridge/bz_tabletop_transport.c client/cl_parse.c platform/tabletop/client/cl_scrn_tabletop_null.c common/net.c common/msg.c,-lm -lpthread,))
...
$(eval $(call test_schema,test-bz-tabletop-audio,,$(TEST_CFLAGS) -Iplatform/bridge,$(BIN_DIR)/test_bz_tabletop_audio$(EXE_EXT),$(WC3_TEST_DIR)/test_bz_tabletop_audio_main.c $(WC3_TEST_DIR)/test_bz_tabletop_audio.c platform/tabletop/client/s_tabletop_null.c,-lm -lpthread,))
$(eval $(call test_schema,test-bz-tabletop-catalog,test-assets,$(TEST_CFLAGS) -Iplatform/bridge,$(BIN_DIR)/test_bz_tabletop_catalog$(EXE_EXT),$(WC3_TEST_DIR)/test_bz_tabletop_catalog_main.c $(WC3_TEST_DIR)/test_bz_tabletop_catalog.c platform/bridge/bz_tabletop_catalog.c $(WC3_DIR)/common/campaign.c common/mpq.c,-lm -lz,))
...
test-assets: blpgen mdxgen mpqtool mdxtool | $(TESTS_DIR)
	...
	@for model in \
		...
		"rigged_anim TestUI/Textures/checker_8x8.blp $(TESTS_RES_DIR)/TestUI/Models/rigged_anim.mdx" \
		"particle_emitter TestUI/Textures/checker_8x8.blp $(TESTS_RES_DIR)/TestUI/Models/particle_emitter.mdx"; do \
...
WC3_PHONY := wc3-build jass-tool jass sheet renderer game ui openwarcraft3 run run-demo run-map run-ui-text test test-jass test-commands test-bz-runtime test-bz-tabletop-lifecycle test-bz-tabletop-transport test-bz-tabletop-assets test-bz-tabletop-audio test-bz-tabletop-catalog test-ui test-mpq-compat test-assets test-render-golden update-render-golden
